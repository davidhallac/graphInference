...
timeShift = timesteps/2 #Number of steps till new covariance matrix appears
eps = 1e-3
...
set_length = 20
alpha_set = np.linspace(0,1,set_length)
beta_set = np.linspace(0,1,set_length)
FroError = []
Score = []
...
numpy.set_printoptions(suppress=True, precision = 3, threshold = 5)
    ...
    for beta in beta_set:
        ...
        #	print empCov
        	...
        	if (i > 0): #Add edge to previous timestamp
        		...
        		edge_obj = beta*norm(currVar['S'] - prevVar['S'],2) # one norm penalty function
...
#        gvx.Solve()
        ...
        for nodeID in range(timesteps):
            ...
            S_est[ind]=0
            S_est = numpy.asarray((S_est + S_est.T) - np.diag(temp))
                
            #Get actual InvCov
            S_actual = 0
            if (nodeID < timeShift):
                S_actual = numpy.asarray(S_true.copy())
            else:
                S_actual = numpy.asarray(S_true2.copy())
                
            D = numpy.where(S_est != 0)[0].shape[0]#len(numpy.where(S_est == 0)[0])
            T = numpy.where(S_actual != 0)[0].shape[0]
            TandD = float(numpy.where(numpy.logical_and(S_actual,S_est) == True)[0].shape[0])
            P = TandD/D
            R = TandD/T
        ...
        #    print numpy.where(numpy.logical_and(S_actual,S_est) == True)[0].shape[0]
            ...
            e2 = e2 + 2* P*R/(P+R)
#            FroError.append(np.linalg.norm(S_actual - np.matrix(S_est), 'fro'))
